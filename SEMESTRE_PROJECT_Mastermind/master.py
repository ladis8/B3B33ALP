...
class master:
    ...
    def generate(self):
        """Prepare random codefor user guess"""
        ...
        if not error:
            for i in range(self.test):  # count black points and mark used position
               if (g[i]==self.orig[i]):
                   ...
                   mask.append(0)
               if not (g[i]>=0 and g[i]<self.glob):  #chect the correctness of the guess
                   error=True

            for i in range(self.test):   # count white points
                if (g[i]!=self.orig[i]): # not black point can be white
                    for j in range(self.test):
                        if (mask[j]==0 and g[i]==self.orig[j]):
                            white+=1
...
if __name__=="__main__":
    ...
    user_mind = mind2.mind()
    master = master(1)   # initialize the game
